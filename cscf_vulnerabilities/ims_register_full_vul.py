...
def build_200_ok_notify(notify_msg):
    """Build 200 OK response to a NOTIFY."""
    # Parse Via, From, To, Call-ID, CSeq from NOTIFY (case-insensitive)
    via = re.search(r'^Via: (.+)$', notify_msg, re.M | re.I)
    ...
    via_val = via.group(1).strip() if via else ""
